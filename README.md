...
## Branding And Custom Logo

DocuSeal can serve a custom logo and generate the matching branding assets for a new installation or white-label deployment.

1. Put your logo file somewhere inside the project or mounted data directory. Any image format supported by the image processing libraries works, but PNG is the simplest option.
2. Update [`config/config.yml`](config/config.yml) and point `logo_png` to the file you want to use.
3. Restart the app or rebuild the container so the branding assets are regenerated.
4. Open [`/branding/logo.png`](http://localhost:3000/branding/logo.png) to verify the logo is being served correctly.
5. If your logo file lives under `test/` in the Docker image, keep this Dockerfile copy step in place so the file is available at runtime: `COPY --chown=docuseal:docuseal ./test ./test`.
...
default:
  name: My Company
  support_url: mailto:support@mycompany.com
  logo_png: test/my-company-logo.png
```

The same branding source is used to generate the favicon and apple touch icon assets under `public/` during startup, so changing `logo_png` updates the whole visual identity in one place.
